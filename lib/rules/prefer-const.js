...
function getWriteReferenceIfShouldBeConst(variable) {
    ...
    for (var i = 0; i < references.length; ++i) {
        ...
        if (reference.isWrite()) {
            if (retv && retv.identifier !== reference.identifier) {
    ...
    // Checks the writer is located at the same scope and can modify it as const.
    if (!retv ||
...
module.exports = function(context) {
    ...
    var shouldWarnMixedCases = options.mixed !== false;
    var variables = null;
    ...
    function checkGroup(writers) {
        // In a mixed case, writers include null.
